...
const DEFAULT_HTML_FORM_TEMPLATE = `<form action="/grazie" method="POST" class="space-y-4">
  <input type="hidden" name="origin" value="landing_page">
  <div>
    <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Nome e Cognome</label>
    <input type="text" name="name" required class="w-full p-3 border border-slate-300 rounded-lg text-sm" placeholder="Inserisci il tuo nome">
  </div>
  <div>
    <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Telefono</label>
    <input type="tel" name="phone" required class="w-full p-3 border border-slate-300 rounded-lg text-sm" placeholder="Cellulare per conferma">
  </div>
  <div>
    <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Indirizzo e N° Civico</label>
    <input type="text" name="address" required class="w-full p-3 border border-slate-300 rounded-lg text-sm" placeholder="Via e Numero Civico">
  </div>
  <div class="grid grid-cols-2 gap-3">
    <input type="text" name="city" required class="w-full p-3 border border-slate-300 rounded-lg text-sm" placeholder="Città">
    <input type="text" name="cap" required class="w-full p-3 border border-slate-300 rounded-lg text-sm" placeholder="CAP">
  </div>

  <div class="p-4 bg-slate-50 rounded-xl border border-slate-200">
    <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Metodo di Pagamento</label>
    <div class="space-y-2">
      <label class="flex items-center gap-2 cursor-pointer p-2 bg-white rounded border border-slate-100">
        ...
        <span class="text-xs font-bold">Pagamento alla Consegna</span>
      </label>
      <label class="flex items-center gap-2 cursor-pointer p-2 bg-white rounded border border-slate-100">
        <input type="radio" name="payment_method" value="card" class="accent-slate-900">
        <span class="text-xs font-bold">Carta di Credito</span>
      </label>
    </div>
  </div>

  <button type="submit" class="w-full bg-orange-500 text-white font-black py-4 rounded-xl shadow-lg mt-2 uppercase tracking-wide">COMPLETA L'ORDINE</button>
</form>`;
...
export const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setTySlug(product.generateThankYou ? (formatSlug(product.name) + '-grazie') : '');
      setIsPublished(false);
    } catch (err: any) {
